...
findPackagePaths() {
  ...
  workspaces=$(for pattern in $patterns; do
    find "$pattern" \( -name "node_modules" -o -name "dist" -o -name "generated" \) -type d -prune -o -name "package.json" -print | xargs -I {} dirname {}
  done)
}
...
updateDependencies() {
  for workspace in $workspaces; do
    ...
    base_dir=$(npm prefix)
    echo "Updating dependencies in $base_dir/$workspace"
